...
class TorchResult(Result):
    ...
    Args:
        ...
        state: The torch tensor representing the state. Can be a statevector
            or a density matrix.
    ...
    def _weights(self) -> Tensor:  # type: ignore [override]
        ...
        if not self.state.shape[1] == 1:
            ...
            probs = (torch.abs(self.state) ** 2).flatten()
    ...
    def get_state(
        ...
        raise NotImplementedError("Not rewritten with torch")

        state = self.state.clone()
        is_density_matrix = state.shape[0] == state.shape[1]

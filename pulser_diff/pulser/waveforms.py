...
from pulser_diff.pulser.json.abstract_repr.serializer import abstract_repr
...
from pulser_diff.pulser.parametrized import Parametrized, ParamObj
from pulser_diff.pulser.parametrized.decorators import parametrize
...
class Waveform(ABC):
    ...
    def __init__(self, duration: Union[int, Parametrized]):
        ...
        if _duration <= 0:
            raise ValueError(
                "A waveform must have a positive duration, " + f"not {duration}."
            )
    ...
    def modulated_samples(self, channel: Channel, eom: bool = False) -> np.ndarray:
    ...
    @functools.lru_cache()
    def _modulated_samples(self, channel: Channel, eom: bool = False) -> np.ndarray:
        """The waveform samples as output of a given channel.
    ...
    def _plot(
        ...
        ax.set_xlabel("t (ns)")
        samples = self.samples if channel is None else self.modulated_samples(channel)
...
class RampWaveform(Waveform):
    ...
    def _samples(self) -> np.ndarray:
        ...
        Returns:
            A numpy array with a value for each time step.
        """
        return (np.arange(1, self._duration + 1) - 1) * (self._stop - self._start) / (
            self._duration - 1
        ) + self._start
    ...
    def _to_abstract_repr(self) -> dict[str, Any]:
        return abstract_repr("RampWaveform", self._duration, self._start, self._stop)
    ...
    def __str__(self) -> str:
        ...
        return f"Ramp({float(self._start):.3g}->{float(self._stop):.3g} rad/µs)"
...
class InterpolatedWaveform(Waveform):
    ...
    def __init__(
        ...
        **interpolator_kwargs: Any,
        ...
        self._data_pts = np.array(
            [
                (np.round(t), v)
                for t, v in zip(self._times * (self._duration - 1), self._values)
            ]
        )
    ...
    def _samples(self) -> np.ndarray:
        ...
        decimals = int(
            min(
                -int(np.log10(np.finfo(samples.dtype).resolution))
                - np.log10(value_range),
                9,
            )
    ...
    def _plot(
        ...
        if not channel:
            ax.scatter(self._data_pts[:, 0] + start_t, self._data_pts[:, 1], c=color)

    def _to_dict(self) -> dict[str, Any]:
        return obj_to_dict(self, self._duration, self._values, **self._kwargs)

    def _to_abstract_repr(self) -> dict[str, Any]:
        if self._kwargs["interpolator"] != "PchipInterpolator" or set(self._kwargs) - {
            "times",
            "interpolator",
        }:

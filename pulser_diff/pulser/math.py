...
class CompBackend:
    ...
    def insert(self) -> Callable:
        ...
        if COMP_BACKEND == "torch":
            ...
            def _insert(t: torch.Tensor, axis: int | None = None) -> torch.Tensor:
    ...
    def norm(self) -> Callable:
        ...
        if COMP_BACKEND == "torch":
            ...
            def _norm(t: lib.Tensor, axis: int = 0) -> lib.Tensor:
                return lib.linalg.norm(lib.as_tensor(t, dtype=lib.float64), dim=axis)
    ...
    def transpose(self) -> Callable:
        ...
        if COMP_BACKEND == "torch":
            ...
            def _transpose(t: lib.Tensor) -> lib.Tensor:
                return lib.transpose(lib.as_tensor(t, dtype=lib.float64), -1, 0)
    ...
    def kaiser(self) -> Callable:
        lib = self.load_lib()
        if COMP_BACKEND == "torch":
            return lambda n, beta: lib.kaiser_window(n, periodic=False, beta=beta)
        elif COMP_BACKEND == "numpy":
            return lib.kaiser
    ...
    def multinomial(self) -> Callable:
        ...
        if COMP_BACKEND == "torch":
            ...
            def _multinomial(n: int, t: lib.Tensor) -> lib.Tensor:
                out = lib.multinomial(lib.as_tensor(t), n, replacement=True).unique(
                    return_counts=True
                )
    ...
    def pad(self) -> Callable:
        ...
        if COMP_BACKEND == "torch":
            ...
            def _pad(
                ...
                if mode == "constant":
                    return lib.pad(t, pad_width, mode, constant_values=constant_values)
                elif mode == "edge":

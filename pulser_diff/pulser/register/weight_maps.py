...
from pulser_diff.pulser.register.traps import COORD_PRECISION, Traps
...
class WeightMap(Traps, RegDrawer):
    ...
    def __init__(
        ...
        if not (np.all(np.array(weights) >= 0) and np.all(np.array(weights) <= 1)):
    ...
    def _hash_object(self) -> hashlib._Hash:
        ...
        sorted_weights = (
            self.sorted_weights.detach().numpy()
            if hasattr(self.sorted_weights, "detach")
            else self.sorted_weights
        )
    ...
    def _to_abstract_repr(self) -> dict[str, Any]:
        d: dict[str, Any] = dict(
            traps=[
                ...
                for weight, (x, y) in zip(self.sorted_weights, self.sorted_coords)
            ]
        )

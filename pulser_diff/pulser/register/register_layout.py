...
import pulser_diff.pulser.json.abstract_repr as pulser_abstract_repr
...
from pulser_diff.pulser.register._reg_drawer import RegDrawer
...
from pulser_diff.pulser.register.traps import Traps
...
class RegisterLayout(Traps, RegDrawer):
    ...
    def define_register(
        ...
        if not trap_ids_set.issubset(self.traps_dict):
            ...
            raise ValueError("All 'trap_ids' must correspond to the ID of a trap.")

        if qubit_ids:
            if len(set(qubit_ids)) != len(qubit_ids):
                raise ValueError("'qubit_ids' must be a sequence of unique IDs.")
        ...
        ids = qubit_ids if qubit_ids else [f"q{i}" for i in range(len(trap_ids))]
        coords = self.sorted_coords[list(trap_ids)]
        qubits = dict(zip(ids, coords))

        reg_class = pulser.Register3D if self.dimensionality == 3 else pulser.Register
    ...
    def from_abstract_repr(obj_str: str) -> RegisterLayout:
        ...
        return pulser_abstract_repr.deserializer.deserialize_abstract_layout(obj_str)

...
class RegDrawer:
    ...
    def _draw_2D(
        ...
        if masked_qubits and dmm_qubits and masked_qubits != set(dmm_qubits.keys()):
        ...
        if with_labels:
            ...
            while i < len(plot_ids):
                ...
                det_map = [q for q, weight in dmm_qubits.items() if weight > 0.0]
                plot_ids[i] = sorted(
                    plot_ids[i],
                ...
                for j in range(len(plot_ids[i])):
                    if plot_ids[i][j] in [str(q) for q in det_map]:
                        ...
                        for q in plot_ids[i][j:]:
                            extra_label = f":{dmm_qubits[q]:.2f}" if not is_mask else ""

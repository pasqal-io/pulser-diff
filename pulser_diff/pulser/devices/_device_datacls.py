...
from pulser_diff.pulser.channels.dmm import DMM
from pulser_diff.pulser.devices.interaction_coefficients import c6_dict
from pulser_diff.pulser.json.abstract_repr.serializer import AbstractReprEncoder
from pulser_diff.pulser.json.abstract_repr.validation import validate_abstract_repr
from pulser_diff.pulser.json.utils import get_dataclass_defaults, obj_to_dict
...
from pulser_diff.pulser.register.traps import COORD_PRECISION
...
class BaseDevice(ABC):
    ...
    def __post_init__(self) -> None:
        def type_check(param: str, type_: type, value_override: Any = None) -> None:
            value = getattr(self, param) if value_override is None else value_override
        ...
        for param in (
            ...
            if param in self._optional_parameters or param in ALWAYS_OPTIONAL_PARAMS:
                prelude = "When defined, "
                is_none = value is None
            elif value is None:
                raise TypeError(
                    f"'{param}' can't be None in a '{type(self).__name__}' " "instance."
                )
            else:
                prelude = ""
                is_none = False
        ...
        if self.supports_slm_mask and not self.dmm_objects:
            raise ValueError("One DMM object should be defined to support SLM mask.")
        ...
        if any(ch.basis == "XY" for ch in self.channel_objects) and not isinstance(
            self.interaction_coeff_xy, float
        ):
            raise TypeError(
    ...
    def dmm_channels(self) -> dict[str, DMM]:
        ...
        return {f"dmm_{i}": dmm_obj for (i, dmm_obj) in enumerate(self.dmm_objects)}
    ...
    def rydberg_blockade_radius(self, rabi_frequency: float) -> float:
        ...
        return cast(float, (self.interaction_coeff / rabi_frequency) ** (1 / 6))
    ...
    def validate_register(self, register: BaseRegister) -> None:
        ...
        if register.dimensionality > self.dimensions:
            raise ValueError(
                f"All qubit positions must be at most {self.dimensions}D " "vectors."
            )
    ...
    def validate_layout_filling(
        ...
        max_qubits = int(register.layout.number_of_traps * self.max_layout_filling)
    ...
    def _validate_atom_distance(
        ...
        def invalid_dists(dists: np.ndarray) -> np.ndarray:
            cond1 = dists - self.min_atom_distance < -(10 ** (-COORD_PRECISION))
        ...
        if len(coords) > 1:
            ...
            if np.any(invalid_dists(distances)):
                ...
                bad_pairs = np.argwhere(np.logical_and(invalid_dists(sq_dists), mask))
    ...
    def _validate_coords(
        ...
        if kind == "atoms" and not (
            "max_atom_num" in self._optional_parameters and self.max_atom_num is None
        ):
...
class Device(BaseDevice):
    ...
    pre_calibrated_layouts: tuple[RegisterLayout, ...] = field(default_factory=tuple)
    ...
    def _specs(self, for_docs: bool = False) -> str:
        ...
        if self.max_sequence_duration is not None:
            lines.append(
                " - Maximum sequence duration: " f"{self.max_sequence_duration} ns"
            )

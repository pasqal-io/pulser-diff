...
import pulser_diff.pulser as pulser
import pulser_diff.pulser.devices as devices
import pulser_diff.pulser.sequence._decorators as seq_decorators
from pulser_diff.pulser.channels.base_channel import Channel
from pulser_diff.pulser.channels.dmm import DMM, _dmm_id_from_name, _get_dmm_name
from pulser_diff.pulser.channels.eom import RydbergEOM
from pulser_diff.pulser.devices._device_datacls import BaseDevice
from pulser_diff.pulser.json.abstract_repr.deserializer import (
    deserialize_abstract_sequence,
)
from pulser_diff.pulser.json.abstract_repr.serializer import serialize_abstract_sequence
from pulser_diff.pulser.json.coders import PulserDecoder, PulserEncoder
from pulser_diff.pulser.json.exceptions import AbstractReprError
from pulser_diff.pulser.json.utils import obj_to_dict
from pulser_diff.pulser.math import CompBackend as np
from pulser_diff.pulser.parametrized import Parametrized, Variable
from pulser_diff.pulser.parametrized.variable import VariableItem
from pulser_diff.pulser.pulse import Pulse
from pulser_diff.pulser.register.base_register import BaseRegister, QubitId
from pulser_diff.pulser.register.mappable_reg import MappableRegister
from pulser_diff.pulser.register.weight_maps import DetuningMap
from pulser_diff.pulser.sequence._basis_ref import _QubitRef
from pulser_diff.pulser.sequence._call import _Call
from pulser_diff.pulser.sequence._schedule import (
    _ChannelSchedule,
    _DMMSchedule,
    _PhaseDriftParams,
    _Schedule,
...
from pulser_diff.pulser.sequence._seq_drawer import Figure, draw_sequence
from pulser_diff.pulser.sequence._seq_str import seq_to_str
from pulser_diff.pulser.waveforms import Waveform
...
class Sequence(Generic[DeviceType]):
    ...
    def __init__(
        ...
        self._schedule: _Schedule = _Schedule(max_duration=device.max_sequence_duration)
    ...
    def _slm_mask_time(self) -> list[int]:
        ...
        return (
            [] if not self._slm_mask_targets else self._schedule.find_slm_mask_times()
        )
    ...
    def qubit_info(self) -> dict[QubitId, np.ndarray]:
        ...
        if self.is_register_mappable():
            raise RuntimeError(
                "Can't access the qubit information when the register is " "mappable."
            )
    ...
    def declared_channels(self) -> dict[str, Channel]:
        ...
        for call in self._to_build_calls:
            if call.name == "config_slm_mask" or call.name == "config_detuning_map":
                ...
                dmm_name = _get_dmm_name(dmm_id, list(all_declared_channels.keys()))
                all_declared_channels[dmm_name] = self.device.dmm_channels[dmm_id]
    ...
    def available_channels(self) -> dict[str, Channel]:
        ...
        if not self._in_xy and not self._in_ising:
            ...
            if self._slm_mask_dmm is not None and not self._device.reusable_channels:
                return {
                    id: ch
    ...
    def get_measurement_basis(self) -> str:
        ...
        return self._param_measurement if self.is_parametrized() else self._measurement
    ...
    def _set_slm_mask_dmm(self, dmm_id: str, targets: set[QubitId]) -> None:
        ...
        if slm_mask_times:
            ...
            cast(_DMMSchedule, self._schedule[key])._waiting_for_first_pulse = True
    ...
    def config_slm_mask(
        ...
        if not self._device.supports_slm_mask:
            raise ValueError(f"The '{self._device}' device does not have an SLM mask.")
            ...
            for new_ch_id, new_ch_obj in all_channels_new_device.items():
                ...
                if not (type_match and basis_match and addressing_match):
                    ...
                    ch_match_err = ch_match_err or (
                        base_msg + " with the right type, basis and addressing."
                    )
                    continue
                ...
                for param_ in params_to_check:
                    if getattr(new_ch_obj, param_) != getattr(old_ch_obj, param_):
                        strict_error_message = base_msg + f" with the same {param_}."
                        break
                else:
    ...
    def declare_channel(
        ...
        if name.startswith("dmm_"):
            raise ValueError("Name starting by 'dmm_' are reserved for DMM channels.")
        ...
        if ch.addressing == "Global":
            ...
            if self.is_parametrized():
                ...
                self._target(cast(Union[Collection, QubitId], initial_target), name)
    ...
    def enable_eom_mode(
        ...
        if self.is_in_eom_mode(channel):
            raise RuntimeError(f"The '{channel}' channel is already in EOM mode.")
        ...
        if not isinstance(on_pulse, Parametrized):
            ...
            if not self.is_parametrized():
                ...
                self._schedule.enable_eom(channel, amp_on, detuning_on, detuning_off)
        ...
        call_container = self._to_build_calls if self.is_parametrized() else self._calls
    ...
    @seq_decorators.store
    @seq_decorators.block_if_measured
    def disable_eom_mode(self, channel: str, correct_phase_drift: bool = False) -> None:
    ...
    def add_eom_pulse(
        ...
        self._add(eom_pulse, channel, protocol, phase_drift_params=phase_drift_params)
    ...
    def align(self, *channels: str) -> None:
        ...
        last_ts = {id: self.get_duration(id, include_fall_time=True) for id in channels}
    ...
    def draw(
        ...
        if mode not in valid_modes:
            raise ValueError(f"'mode' must be one of {valid_modes}, not '{mode}'.")
        if mode == "output":
            if draw_phase_area:
                warnings.warn(
        ...
        if draw_register and self.is_register_mappable():
            raise ValueError(
                "Can't draw the register for a sequence without a defined " "register."
            )
    ...
    def _modulate_slm_mask_dmm(self, duration: int, max_amp: float) -> None:
        if self._slm_mask_dmm is not None:
            ...
            if (
                total_bottom_detuning
                and min_det * len(set(self._slm_mask_targets)) < total_bottom_detuning
            ):
                min_det = total_bottom_detuning / len(set(self._slm_mask_targets))
            cast(
                _DMMSchedule, self._schedule[self._slm_mask_dmm]
    ...
    def _add(
        ...
        ph_refs = {self._basis_ref[basis][q].phase.last_phase for q in last.targets}
        if len(ph_refs) != 1:
            raise ValueError(
    ...
    def _target(
        ...
        if not qubits_set:
            raise ValueError("Need at least one qubit to target but none were given.")
        ...
        if not self.is_parametrized():
            basis = channel_obj.basis
            phase_refs = {
                self._basis_ref[basis][q].phase.last_phase for q in qubit_ids_set
            }
            if len(phase_refs) != 1:
                raise ValueError(
    ...
    def _check_qubits_give_ids(
        ...
        if _index:
            if self.is_parametrized():
                ...
                try:
                    return {self._register.qubit_ids[index] for index in qubits}
                except IndexError:
    ...
    def _get_last_eom_pulse_phase_drift(self, channel: str) -> _PhaseDriftParams:
        eom_settings = self._schedule[channel].eom_blocks[-1]
        try:
            last_pulse_tf = (
                self._schedule[channel].last_pulse_slot(ignore_detuned_delay=True).tf
            )
        except RuntimeError:
    ...
    def _validate_and_adjust_pulse(
        ...
        if channel in self._schedule:
            ...
            if isinstance(channel_obj, DMM):
                ...
                detuning_map = cast(_DMMSchedule, self._schedule[channel]).detuning_map
        else:
            ...
            for call in self._calls[1:] + self._to_build_calls:
                if call.name == "config_detuning_map" or call.name == "config_slm_mask":

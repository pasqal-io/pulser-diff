...
from pulser_diff.pulser import Register, Register3D
...
from pulser_diff.pulser.sampler.sampler import sample
from pulser_diff.pulser.sampler.samples import (
    ChannelSamples,
    DMMSamples,
    SequenceSamples,
)
...
class ChannelDrawContent:
    ...
    def _give_curves_from_samples(self, samples: ChannelSamples) -> list[np.ndarray]:
        return [
            getattr(samples, self._samples_from_curves[qty]) for qty in CURVES_ORDER
        ]
...
def gather_data(
    ...
    for ch, ch_samples in sampled_seq.channel_samples.items():
        ...
        if len(eom_intervals) > 0 and ch_samples.duration == eom_intervals[-1].tf:
            ...
            if isinstance(ch_data.samples, DMMSamples):
                ...
                det_weight_map = defaultdict(
                    ...
                    det_map.get_qubit_weight_map(
                        ch_data.samples.qubits if not register else register.qubits
                    ),
                )
                    ...
                    for w, set_t in weight_target_map[i].items():
                        ...
                        if sub_target != set():
                            pad_times = (
                                ...
                                0 if times == "initial" else total_duration - target[1],
                            )
                            qubit_data[i][tuple(sub_target)] = (
                                w * np.pad(ch_samples[:total_duration], pad_times)
                                + q_data
                            )
    return qubit_data
...
def _draw_register_det_maps(
    ...
    nregisters = int(register is not None) + int(draw_detuning_maps) * n_det_maps
    # Draw masked register
    if register:
        pos = np.array(register._coords)
        title = (
            "Register" if sampled_seq._slm_mask.targets == set() else "Masked register"
        )
    ...
    if draw_detuning_maps:
        ...
        for i, (ch, det_map) in enumerate(det_maps.items()):
            ...
            if need_init:
                ...
                need_init = False
            ax_reg = (
                axes_reg if nregisters == 1 else axes_reg[i + int(register is not None)]
            )
            if det_map.dimensionality == 3:
                for sub_ax_reg, (ix, iy) in zip(ax_reg, combinations(np.arange(3), 2)):
                    det_map._draw_2D(
                        ax=sub_ax_reg,
                        pos=pos,
...
def _draw_channel_content(
    ...
    for i, (ch, gs_) in enumerate(zip(sampled_seq.channels, gs)):
        ...
        ch_axes[ch] = [fig.add_subplot(subgs[i, :]) for i in range(data[ch].n_axes_on)]
    ...
    for ch, axes in ch_axes.items():
        ...
        ys_mod = [()] * 3
        yseff = [()] * 3
        draw_output = draw_modulation and (ch_obj.mod_bandwidth or not draw_input)
        if draw_output:
        ...
        ref_ys = [
            list(chain.from_iterable(all_ys)) for all_ys in zip(ys, ys_mod, yseff)
        ]
        ...
        if draw_phase_area:
            ...
            for slot in ch_data.samples.slots:
                if sampling_rate:
                    area_val = np.sum(yseff[0][slot.ti : slot.tf]) * 1e-3 / np.pi
                else:
                    area_val = (
                        np.sum(ch_data.samples.amp[slot.ti : slot.tf]) * 1e-3 / np.pi
                    )
                phase_val = ch_data.samples.phase[slot.tf - 1]
                x_plot = (slot.ti + slot.tf) / 2 / time_scale
                target_slot_tf_list = [
                    target_slot.tf
                    for target_slot in sampled_seq.channel_samples[ch].target_time_slots
                ]
...
def _draw_qubit_content(
    ...
    qubit_data = [
        gather_qubit_data(sampled_seq, data, register, (data_name == "modulated"))
        if to_draw
        else None
    ...
    if register or dmm_samples:
        dimensionality_3d = isinstance(register, Register3D) or (
            dmm_samples != []
            and cast(DetuningMap, dmm_samples[0].detuning_map).dimensionality == 3
        )
...
def draw_samples(
    ...
    slot_tfs = [
        ch_samples.slots[-1].tf for ch_samples in sampled_seq.channel_samples.values()
    ]
...
def draw_sequence(
    ...
    if not draw_output and not draw_input and (draw_qubit_det or draw_qubit_amp):
        warnings.warn(

...
from pulser_diff.pulser.channels import DMM
from pulser_diff.pulser.pulse import Pulse

if TYPE_CHECKING:
    from pulser_diff.pulser.sequence.sequence import Sequence
...
def seq_to_str(sequence: Sequence) -> str:
    ...
    for ch, seq in sequence._schedule.items():
        if seq.channel_obj.addressing == "Global" and sequence.is_register_mappable():
            warnings.warn(
            ...
            if isinstance(ts.type, Pulse):
                if isinstance(sequence.declared_channels[ch], DMM):
                    full += dmm_det_line.format(
                        ...
                        else "{:.3g} rad/µs".format(cast(float, ts.type.detuning[0])),
                        tgt_txt,
                    )

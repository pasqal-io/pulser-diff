...
class _ChannelSchedule:
    ...
    def in_eom_mode(self, time_slot: Optional[_TimeSlot] = None) -> bool:
        ...
        return any(
            start <= time_slot.ti < end for start, end in self.get_eom_mode_intervals()
        )
    ...
    def get_samples(self, ignore_detuned_delay_phase: bool = True) -> ChannelSamples:
        ...
        for ind, s in enumerate(channel_slots):
            ...
            for last_pulse_ind in range(ind - 1, -1, -1):  # From ind-1 to 0
                ...
                if not (
                    ...
                    and self.is_detuned_delay(cast(Pulse, last_pulse_slot.type))
                ):
        ...
        for s in self.slots:
            ...
            if (
                ...
                and det[s.tf - 1] == self.eom_blocks[eom_block_n + 1].detuning_off
            ):
    ...
    def __getitem__(self, key: Union[int, slice]) -> Union[_TimeSlot, list[_TimeSlot]]:
...
class _DMMSchedule(_ChannelSchedule):
    ...
    def get_samples(
        ...
        init_fields = {
            f.name: getattr(ch_samples, f.name) for f in fields(ch_samples) if f.init
        }
        if qubits is None:
            raise ValueError(
        ...
        return DMMSamples(**init_fields, detuning_map=self.detuning_map, qubits=qubits)
...
class _Schedule(Dict[str, _ChannelSchedule]):
    ...
    def find_slm_mask_times(self) -> list[int]:
        ...
        for ch_schedule in self.values():
            ...
            for slot in ch_schedule:
                if not isinstance(slot.type, Pulse) or ch_schedule.is_detuned_delay(
                    slot.type
                ):
                    continue
                ti = slot.ti
    ...
    def add_pulse(
        ...
        def corrected_phase(tf: int) -> float:
            phase_drift = (
                phase_drift_params.calc_phase_drift(tf) if phase_drift_params else 0
            )
        ...
        if protocol != "no-delay":
            current_max_t = self._find_add_delay(current_max_t, channel, protocol)
            try:
    ...
    def add_delay(self, duration: int, channel: str) -> None:
        ...
        if (
            ...
            self[channel].slots.append(_TimeSlot(delay_pulse, ti, tf, last.targets))
        else:
    ...
    def add_target(self, qubits_set: set[QubitId], channel: str) -> None:
        ...
        if self[channel].slots:
            ...
            tf = 0
        self._check_duration(tf)
        self[channel].slots.append(_TimeSlot("target", ti, tf, set(qubits_set)))
        ...
        for ch, ch_schedule in self.items():
            ...
            for op in ch_schedule[::-1]:
                if not isinstance(op.type, Pulse):
                    ...
                    op.tf + op.type.fall_time(this_chobj, in_eom_mode=in_eom_mode)
                    <= current_max_t
                ):
                    break
                elif (
                    op.targets & self[channel][-1].targets or protocol == "wait-for-all"
                ):

...
from pulser_diff.pulser.channels.eom import BaseEOM
...
if TYPE_CHECKING:
    from pulser_diff.pulser.sequence._schedule import _EOMSettings, _TimeSlot
...
class ChannelSamples:
    ...
    def _generate_std_samples(self) -> ChannelSamples:
        new_samples = {key: np.copy(getattr(self, key)) for key in ("amp", "det")}
    ...
    def in_eom_mode(self, slot: _TimeSlot | _PulseTargetSlot) -> bool:
        ...
        return any(
            start <= slot.ti < end for start, end in self.get_eom_mode_intervals()
        )
    ...
    def modulate(
            ...
            if keep_end_values:
                ...
                for reg in masked_regions:
                    ...
                    if reg[0] > 0:
                        ...
                        new_samples[reg[0] : reg[0] + delta // 2] = samples[reg[0]]
            else:
                new_samples[~mask] = 0
            return new_samples
        ...
        eom_samples = {key: np.copy(getattr(self, key)) for key in ("amp", "det")}
        ...
        if self.eom_blocks:
            ...
            for key in ("amp", "det"):
                ...
                buffers = masked(modulated_buffer[: len(std)], eom_buffers_mask)
                ...
                if key == "det":
                    ...
                    samples_[eom_mask_ext] = modulated_std[: len(eom_mask_ext)][
                        eom_mask_ext
                    ]
                    ...
                    modulated_eom = channel_obj.modulate(eom_samples[key], eom=True)
                ...
                for arr in sample_arrs[:-1]:
                    n1 = (
                        sample_arrs[-1].numel()
                        if hasattr(sample_arrs[-1], "detach")
                        else sample_arrs[-1].size
                    )
...
class SequenceSamples:
    ...
    _basis_ref: dict[str, dict[QubitId, _QubitRef]] = field(default_factory=dict)
    ...
    def used_bases(self) -> set[str]:
        ...
        return {
            ch_obj.basis
            for ch_obj, ch_samples in zip(self._ch_objs.values(), self.samples_list)
    ...
    def extend_duration(self, new_duration: int) -> SequenceSamples:
        ...
        return replace(
            self,
            samples_list=[
                sample.extend_duration(new_duration) for sample in self.samples_list
            ],
        )
    ...
    def __repr__(self) -> str:
        blocks = [
            f"{chname}:\n{cs!r}" for chname, cs in zip(self.channels, self.samples_list)
        ]
        return "\n\n".join(blocks)

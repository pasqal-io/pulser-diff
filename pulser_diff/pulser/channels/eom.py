...
class BaseEOM(_BaseEOMDefaults, _BaseEOM):
    ...
    def __post_init__(self) -> None:
        ...
        if self.custom_buffer_time is not None and int(self.custom_buffer_time) <= 0:
            raise ValueError(
    ...
    def _to_abstract_repr(self) -> dict[str, Any]:
        ...
        for f in all_fields:
            ...
            if f.name in OPTIONAL_ABSTR_EOM_FIELDS and value == defaults[f.name]:
                continue
            params[f.name] = value
        return params
...
class RydbergEOM(_RydbergEOMDefaults, BaseEOM, _RydbergEOM):
    ...
    def __post_init__(self) -> None:
        ...
        for param in ["max_limiting_amp", "intermediate_detuning"]:
            ...
            if value <= 0.0:
                raise ValueError(f"'{param}' must be greater than zero, not {value}.")
        if not isinstance(self.controlled_beams, tuple):
            ...
            object.__setattr__(self, "controlled_beams", tuple(self.controlled_beams))
        if not self.controlled_beams:
            raise ValueError("There must be at least one beam in 'controlled_beams'.")
    ...
    def detuning_off_options(
        ...
        if len(self.controlled_beams) == 1:
            ...
            # switched off) on.
            lightshifts = [self._lightshift(rabi_frequency, ~self.controlled_beams[0])]

        else:
            # When both beams are controlled, we have three options for the
            # lightshift: (ON, OFF), (OFF, ON) and (OFF, OFF)
            lightshifts = [
                self._lightshift(rabi_frequency, beam) for beam in self.controlled_beams
            ]
    ...
    def _lightshift(self, rabi_frequency: float, *beams_on: RydbergBeam) -> float:
    ...
    def _rabi_freq_per_beam(self, rabi_frequency: float) -> dict[RydbergBeam, float]:
        # rabi_freq = (rabi_red * rabi_blue) / (2 * int_detuning)
        limit_rabi_freq = self.max_limiting_amp**2 / (2 * self.intermediate_detuning)

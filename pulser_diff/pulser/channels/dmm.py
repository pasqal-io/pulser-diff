...
class DMM(Channel):
    ...
    def __post_init__(self) -> None:
        ...
        if self.total_bottom_detuning:
            ...
            if (
                ...
                raise ValueError(
                    "total_bottom_detuning must be lower than" " bottom_detuning."
                )
    ...
    def validate_pulse(
        ...
        if (
            ...
            and np.max(detuning_map.weights) * min_round_detuning < self.bottom_detuning
        ):
            raise ValueError(
...
def _get_dmm_name(dmm_id: str, channels: list[str]) -> str:
    ...
    """
    dmm_count = len([key for key in channels if _dmm_id_from_name(key) == dmm_id])
    if dmm_count == 0:
        return dmm_id

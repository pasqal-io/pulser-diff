...
from pulser_diff.pulser.channels.eom import MODBW_TO_TR, BaseEOM
...
class Channel(ABC):
    ...
    def __post_init__(self) -> None:
        ...
        for param in parameters:
            ...
            if param in optional:
                ...
                raise TypeError(
                    f"'{param}' can't be None in a '{self.addressing}' " "channel."
                )
            else:
                prelude = ""
                valid = False
        ...
        if self.mod_bandwidth is not None and self.mod_bandwidth > MODBW_TO_TR * 1e3:
    ...
    def validate_pulse(self, pulse: Pulse) -> None:
        ...
        if self.max_amp is not None and np.any(pulse.amplitude.samples > self.max_amp):
            raise ValueError(
        ...
        if self.max_abs_detuning is not None and np.any(
            np.round(np.abs(pulse.detuning.samples), decimals=6) > self.max_abs_detuning
        ):
            raise ValueError(
    ...
    def modulate(
        ...
        if keep_ends:
            samples = np.pad(input_samples, mod_padding + self.rise_time, mode="edge")
        else:
    ...
    @staticmethod
    def apply_modulation(input_samples: np.ndarray, mod_bandwidth: float) -> np.ndarray:
        ...
        size = (
            input_samples.numel()
            if hasattr(input_samples, "detach")
            else input_samples.size
        )
        freqs = np.fftfreq(size)
        modulation = np.exp(-(freqs**2) / fc**2)
        return cast(np.ndarray, np.ifft(np.fft(input_samples) * modulation).real)
    ...
    def _eom_buffer_time(self) -> int:
        ...
        return int(
            cast(BaseEOM, self.eom_config).custom_buffer_time or 2 * self.rise_time
        )
    ...
    def _to_dict(self, _module: str = "pulser.channels") -> dict[str, Any]:
        params = {f.name: getattr(self, f.name) for f in fields(self) if f.init}

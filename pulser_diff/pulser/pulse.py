...
from pulser_diff.pulser.waveforms import ConstantWaveform, Waveform
...
class Pulse:
    ...
    def __init__(
        ...
        if not (isinstance(amplitude, Waveform) and isinstance(detuning, Waveform)):
    ...
    def ConstantDetuning(
        ...
        """
        detuning_wf = ConstantWaveform(cast(Waveform, amplitude).duration, detuning)
    ...
    def ConstantAmplitude(
        ...
        Args:
            ...
            post_phase_shift: Optionally lets you add a
                phase shift (in rads) immediately after the end of the pulse.
        """
        amplitude_wf = ConstantWaveform(cast(Waveform, detuning).duration, amplitude)
    ...
    def fall_time(self, channel: Channel, in_eom_mode: bool = False) -> int:
        ...
        aligned_start_extra_time = (
            channel.rise_time
            if not in_eom_mode
            else cast(pulser.channels.eom.BaseEOM, channel.eom_config).rise_time
        )
        end_extra_time = max(

...
import pulser_diff.pulser.parametrized
...
if TYPE_CHECKING:
    from pulser_diff.pulser.parametrized import Variable
...
class ParamObj(Parametrized, OpSupport):
    ...
    def _to_dict(self) -> dict[str, Any]:
        def class_to_dict(cls: Callable) -> dict[str, Any]:
            ...
            return obj_to_dict(self, _build=False, _name=cls.__name__, _module=module)

        args = list(self.args)
        if isinstance(self.cls, Parametrized):
            raise ValueError(
                "Serialization of calls to parametrized objects is not " "supported."
            )
    ...
    def _to_abstract_repr(self) -> dict[str, Any]:
        ...
        if (
            ...
            if inspect.isclass(self.args[0]):
                ...
                signature = SIGNATURES["Pulse" if cls_name == "Pulse" else name]
            ...
            if op_name == "InterpolatedWaveform" and all_args["times"] is None:
                if isinstance(
                    all_args["values"],
                    pulser_diff.pulser.parametrized.Variable,  # Avoids circular import
                ):
            ...
            raise AbstractReprError(f"No abstract representation for '{op_name}'.")

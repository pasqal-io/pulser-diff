...
from pulser_diff.pulser.json.abstract_repr.signatures import SIGNATURES
...
from pulser_diff.pulser.json.utils import stringify_qubit_ids
from pulser_diff.pulser.math import CompBackend as np

if TYPE_CHECKING:
    from pulser_diff.pulser.parametrized import Parametrized
...
def abstract_repr(name: str, *args: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    if len(args) < len(signature.pos):
        ...
        if signature.var_pos is not None or not set(arg_as_kwarg) <= set(kwargs):
            raise ValueError(
    ...
    max_pos_args = len(signature.pos) + len(set(signature.keyword) - set(kwargs))
...
def serialize_abstract_sequence(
    ...
    **defaults: Any,
    ...
    def remove_kwarg_if_default(
        ...
        if data.get(kwarg_name, None) == get_kwarg_default(call_name, kwarg_name):
    ...
    for call in chain(seq._calls, seq._to_build_calls):
        if call.name == "__init__":
            ...
            data = get_all_args(("channel", "channel_id", "initial_target"), call)
            ...
            if call.name == "target":
                ...
                if isinstance(data["qubits"], pulser.parametrized.Parametrized):
                    # The qubit indices are given through a variable
                    target = data["qubits"]
                else:
                    # Either a single index or a sequence of indices
                    target = cast(Union[int, list], unfold_targets(data["qubits"]))
            else:
            ...
            if seq._in_xy and data["dmm_id"] == get_kwarg_default(call.name, "dmm_id"):
            ...
            data = remove_kwarg_if_default(data, call.name, "correct_phase_drift")
    ...
    abstr_seq_str = json.dumps(res, cls=AbstractReprEncoder, **json_dumps_options)

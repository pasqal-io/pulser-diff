...
from pulser_diff.pulser.channels import DMM, Microwave, Raman, Rydberg
from pulser_diff.pulser.channels.base_channel import Channel
from pulser_diff.pulser.channels.eom import (
    OPTIONAL_ABSTR_EOM_FIELDS,
    RydbergBeam,
    RydbergEOM,
)
from pulser_diff.pulser.devices import Device, VirtualDevice
from pulser_diff.pulser.devices._device_datacls import PARAMS_WITH_ABSTR_REPR
from pulser_diff.pulser.json.abstract_repr.signatures import (
    BINARY_OPERATORS,
    UNARY_OPERATORS,
)
from pulser_diff.pulser.json.abstract_repr.validation import validate_abstract_repr
from pulser_diff.pulser.json.exceptions import AbstractReprError, DeserializeDeviceError
from pulser_diff.pulser.json.utils import get_dataclass_defaults
from pulser_diff.pulser.parametrized import ParamObj, Variable
from pulser_diff.pulser.pulse import Pulse
from pulser_diff.pulser.register.mappable_reg import MappableRegister
from pulser_diff.pulser.register.register_layout import RegisterLayout
...
if TYPE_CHECKING:
    ...
    from pulser_diff.pulser.sequence import Sequence
...
@overload
def _deserialize_parameter(param: list[int], vars: dict[str, Variable]) -> list[int]:
    pass
...
def _deserialize_parameter(
    ...
    expression = param["expression"] if param["expression"] != "div" else "truediv"
...
def _deserialize_waveform(obj: dict, vars: dict) -> Waveform:
    ...
    if obj["kind"] == "custom":
        return CustomWaveform(samples=_deserialize_parameter(obj["samples"], vars))
    ...
    if op["op"] == "target":
        ...
        if (
            ...
            pulse = Pulse.ConstantAmplitude(
                amplitude=_deserialize_parameter(op["amplitude"]["value"], vars),
...
def _deserialize_channel(obj: dict[str, Any]) -> Channel:
    ...
    if obj["basis"] == "ground-rydberg":
        ...
        if obj["eom_config"] is not None:
            ...
            try:
                optional = {
                    key: data[key] for key in OPTIONAL_ABSTR_EOM_FIELDS if key in data
                }
                ...
                raise AbstractReprError("RydbergEOM deserialization failed.") from e
...
def _deserialize_layout(layout_obj: dict[str, Any]) -> RegisterLayout:
    try:
        return RegisterLayout(layout_obj["coordinates"], slug=layout_obj.get("slug"))
    except ValueError as e:
        raise AbstractReprError("Register layout deserialization failed.") from e
...
def _deserialize_device_object(obj: dict[str, Any]) -> Device | VirtualDevice:
    ...
    for param in device_fields:
        ...
        if param.name == "pre_calibrated_layouts":
            ...
            params[key] = tuple(_deserialize_layout(layout) for layout in obj[key])
        else:
            params[param.name] = obj[param.name]
    try:
...
def deserialize_abstract_sequence(obj_str: str) -> Sequence:
    ...
    if {"name", "x", "y"} == qubits[0].keys():
        ...
        # Mappable register
        assert layout is not None, "Layout must be defined in a MappableRegister."
...
def deserialize_device(obj_str: str) -> Device | VirtualDevice:
    ...
    if not isinstance(obj_str, str):
        type_error = TypeError(f"'obj_str' must be a string, not {type(obj_str)}.")

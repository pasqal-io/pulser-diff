...
from pulser_diff.pulser.backend.abc import Backend
from pulser_diff.pulser.backend.config import EmulatorConfig
from pulser_diff.pulser_simulation.simconfig import SimConfig
from pulser_diff.pulser_simulation.simresults import SimulationResults
from pulser_diff.pulser_simulation.simulation import QutipEmulator
...
class QutipBackend(Backend):
    ...
    def __init__(self, sequence: Sequence, config: EmulatorConfig = EmulatorConfig()):
        """Initializes a new QutipBackend."""
        super().__init__(sequence)
        if not isinstance(config, EmulatorConfig):
            raise TypeError(
                "'config' must be of type 'EmulatorConfig', " f"not {type(config)}."
            )
        self._config = config

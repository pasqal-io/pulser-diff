...
class Hamiltonian:
    ...
    def _extract_samples(self) -> None:
        ...
        def add_noise(
            ...
            """
            noise_amp_base = max(0, np.random.normal(1.0, self.config.amp_sigma))
    ...
    def _construct_hamiltonian(self, update: bool = True) -> None:
        ...
        def make_xy_term(q1: QubitId, q2: QubitId) -> qutip.Qobj:
            ...
            mag_field = cast(np.ndarray, self.samples_obj._magnetic_field)[:coords_dim]
            ...
            return U * self.build_operator([("sigma_ud", [q1]), ("sigma_du", [q2])])
        ...
        def build_coeffs_ops(basis: str, addr: str) -> list[list]:
            ...
            if addr == "Global":
                ...
                for op_id, coeff in zip(op_ids, coeffs):
                    coeff = (
                        coeff.detach().numpy() if hasattr(coeff, "detach") else coeff
                    )
                    if np.any(coeff != 0):

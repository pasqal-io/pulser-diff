...
from pulser_diff.pulser.sequence._seq_drawer import draw_samples, draw_sequence
from pulser_diff.pulser_simulation.hamiltonian import Hamiltonian
from pulser_diff.pulser_simulation.qutip_result import QutipResult
from pulser_diff.pulser_simulation.simconfig import SimConfig
from pulser_diff.pulser_simulation.simresults import (
    CoherentResults,
    NoisyResults,
    SimulationResults,
)
...
class QutipEmulator:
    ...
    def set_initial_state(self, state: Union[str, np.ndarray, qutip.Qobj]) -> None:
        ...
        if set(self.config.noise).issubset(
            ...
            if "SPAM" not in self.config.noise or self.config.eta == 0:
                ...
                initial_configs = Counter(
                    "".join(
                        (
                            np.random.uniform(size=len(self._hamiltonian._qid_index))
                            < self.config.eta
                        )
                        .astype(int)
